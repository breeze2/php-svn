...
class SvnConfig
{
    ...
    private function parse()
    {
        if (!file_exists($this->configFilePath) || !is_file($this->configFilePath)) {
            throw new SvnException('Config file does not exist. ' . $this->configFilePath);
        }

        $fh = fopen($this->configFilePath, 'r');
        if (!flock($fh, LOCK_SH)) {
            throw new SvnException('Can not lock (shared) file. ' . $this->configFilePath);
        }
    ...
    public function save($path = null)
    {
        ...
        if (!file_exists($path)) {
            ...
            if (!touch($path)) {
                throw new SvnException('File does not exist and can not create it. ' . $path);
            }
        }
